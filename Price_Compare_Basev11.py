"""
Incorporated end-user feedback. Changed labels to be more specific
Changed key binds to allow the tab key to switch between widgets
Allowed user to be able to change the price range
"""
...
class Root(tk.Tk):
    def __init__(self):
        ...
        s.configure("BigLabel.TLabel", font=("Ariel", 11))
        s.configure("PadFrame.TFrame", background="#97dbe5")
        s.configure("Instruction.TLabel", font=("Calibri", 8),
                    foreground="#64676b")
        ...
        self.good_entry.grid(row=3, column=2, sticky="n, w")
        # Info label
        self.good_entry_info = ttk.Label(self.mainframe, text="Item name: ")
        self.good_entry_info.grid(row=3, column=1, sticky="n")
        # Instuction label
        self.good_entry_instruction = ttk.Label(self.mainframe,
                                                text="You can press Enter to"
                                                     " go to the next box",
                                                style="Instruction.TLabel")
        self.good_entry_instruction.grid(row=4, column=2, sticky="n")
        # Frame for all amount of units widgets
        self.amount_of_units_frame = ttk.Frame(self.mainframe)
        self.amount_of_units_frame.grid(row=5, column=1, columnspan=3,
                                        sticky="w")
        ...
        self.price_frame.grid(row=6, column=1, columnspan=2, sticky="w")
        ...
        self.push_info_button.grid(row=7, column=1, sticky="s", columnspan=5)
        self.push_info_button.grid_configure(padx=5, pady=(15, 0))
        ...
        self.price_range_mainlabel.grid(row=0, column=0, columnspan=2,
                                        sticky="e")
        # Button to change price range
        self.price_range_change = ttk.Button(self.mainframe, text="Change",
                                             command=lambda: self.change_range(
                                                 ))
        self.price_range_change.grid(row=0, column=1, columnspan=3,
                                     pady=10)
        # Listbox component
        ...
        self.scroller.grid(row=1, column=0, sticky="n, e, s", rowspan=6,
                           padx=10, pady=10)
        ...
        self.item_delete_button.grid(row=7, column=0, sticky="s")
        # Instructions for delete button
        self.item_delete_instructions = ttk.Label(self.mainframe,
                                                  text="You can select an "
                                                       "item\n from the list "
                                                       "above\n then click "
                                                       "delete to remove it",
                                                  style="Instruction.TLabel")
        self.item_delete_instructions.grid(row=8, column=0, sticky="n")
        # Error for calculate button
        self.calculate_error = ttk.Label(self.mainframe, style="Error.TLabel")
        self.calculate_error.grid(row=9, column=0, columnspan=5)
        # Calculate button
        self.calculate = ttk.Button(self.mainframe, text="Calculate!",
                                    command=lambda: self.make_result())
        self.calculate.grid(row=10, column=0, columnspan=5)
        # Enter key bindings
    ...
    def make_result(self):
        ...
        for result in items:
            if result.price > self.price_range:
                ...
                ins.append(result)
        # Sort out the ones that are out of the price range
        # and make the response based on how many there are
        if len(outs) < 1:
            first_part = ""
        elif len(outs) == 1:
            best_out = [outs[0]]
            first_part = f"The best buy is {best_out[0].amount}" \
                         f"{self.selected.get()} of " \
                         f"{best_out[0].name}, however, this is greater than" \
                         f" your maximum price \n"
        else:
            first_part = ""
            best_out = [outs[0]]
            for item in outs[1:]:
                ...
                for out in best_out:
                    first_part += f" {out.amount}{self.selected.get()} of" \
                                  f" {out.name} and"
                first_part = first_part[:-4]
                if len(best_out) > 1:
                    first_part += ", with the same value for money, " \
                                  "however, this is greater than your" \
                                  " maximum price.\n"
                else:
                    first_part += ", however, this is greater than your" \
                                  " maximum price\n"
        ...
        if len(ins) < 1:
            ...
            best_in = [ins[0]]
            second_part = f"The best buy in your budget is" \
                          f" {best_in[0].amount}{self.selected.get()} of " \
                          f"{best_in[0].name}"
        else:
            best_in = [ins[0]]
            for item in ins[1:]:
            ...
            for inner in best_in:
                second_part += f" {inner.amount}{self.selected.get()} of" \
                               f" {inner.name} and"
...
class FirstTopLevel(tk.Toplevel):
    def __init__(self, parent):
        ...
        # Label for info
        self.price_range_label = ttk.Label(self.info_frame,
                                           text="Maximum price:  $")
...
class LastTopLevel(tk.Toplevel):
    def __init__(self, parent, first_part, second_part):
        ...
        # Title label
        ttk.Label(self.result_frame, text="Best Buy:",
                  font=("Ariel", 15)).grid(row=0, column=0)
        # Labels for result
        # If the label is empty don't show it
        if first_part != "":
